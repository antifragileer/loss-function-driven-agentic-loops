...
When you have dispatched async work (a subagent running a
loss-function loop, a background shell wrapping Cline) and
need to verify it is making progress, run these checks in
order. None modify state.

This is the cross-reference from `cline-orchestration`'s
...
```bash
ps auxww | grep -E "agent|cline|hermes" | grep -v grep | head -10
```

If empty: the worker never started or it crashed.

## Step 2 — Did the worker write its expected files?

```bash
ls -la <project>/skills/cline-driver/ 2>/dev/null
ls -la <project>/logs/ 2>/dev/null
```

If empty: worker crashed mid-task.
