...
| `lfd-thinking-protocols` | 0.4.0 | gate (10 thinking protocols the meta-skill invokes between Rounds 0-7; ships `references/frameworks.md` so the skill is self-contained without the LFD repo) | optional — meta-session helper, not loaded by the loop session |
...
*expected* to work but are *not verified* as of v2.4.0.
